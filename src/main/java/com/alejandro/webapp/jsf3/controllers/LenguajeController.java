...
public class LenguajeController implements Serializable {
    ...
    public void init(){
        ...
        lenguajesSoportados = new HashMap<>(Map.of(
                "Ingles", "en",
                "Español", "es"
        ));
    }
    ...
    public void seleccionar(ValueChangeEvent event){
        ...
        lenguajesSoportados.entrySet().stream()
                .filter(entry -> entry.getValue().equals(nuevo))
                .findFirst()
                .ifPresent(entry -> {
                    this.locale = new Locale(entry.getValue());
